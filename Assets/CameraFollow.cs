...
public class CameraFollow : MonoBehaviour
{
    ...
    void Start()
    {
        bc2d = target.GetComponent<BoxCollider2D>();
        playerMovement = target.GetComponent<PlayerMovement>();
        focusArea = new FocusArea(bc2d.bounds, focusAreaSize);
    }

    void LateUpdate()
    {
        focusArea.Update(bc2d.bounds);
        ...
        if (focusArea.velocity.x != 0)
        {
            ...
            if (Mathf.Sign(playerMovement.movement.x) == Mathf.Sign(focusArea.velocity.x) && playerMovement.movement.x != 0)
            {
